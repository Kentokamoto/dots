        ...
        !is_tpopup_new)
    {
        ...
        $container.hide().html(
            ...
            '<div id="footer" class="popup-container popup-footer">'+
                ...
                '</div>'+
            '</div>');
        load_css(base_url+'/css/popup.css'+qs, function(){
            update_ui();
        ...
        reload_css('css/popup.css', function(){
...
define(['jquery', 'underscore', '/bext/pub/backbone.js', '/util/etask.js',
    ...
    !window.is_tpopup ? '/bext/vpn/pub/ui_accel.js' : undefined,
    ...
    'config', '/util/storage.js', '/bext/vpn/pub/user_nav.js',
    '/bext/vpn/pub/social.js', 'backbone', '/util/escape.js'],
    function($, _, be_backbone, etask, zerr, B, be_ui_vpn, ui_accel,
...
function render_tabs(){
    ...
    var is_accel = ['firefox', 'opera'].includes(browser) ||
        window.zon_config.CONFIG_MP_CHROME && browser=='chrome';
    var tabs = [{label: 'vpn', pos: 0, on_show:
        be_ui_vpn.on_show.bind(be_ui_vpn), on_hide:
        be_ui_vpn.on_hide.bind(be_ui_vpn)}];
    if (is_accel)
        tabs.push({label: 'accel', pos: 1});
    if (E.tabs_view)
        E.tabs_view.remove();
    ...
    if (is_accel)
    {
        E.listenToOnce(ui_accel, 'render_done', function(){
            tabs_view.get_tab(1).$content.append(ui_accel.$el); });
    }
